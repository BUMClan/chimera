...
namespace Chimera {
    bool camo_fix_command(int argc, const char **argv, const char **dart) {
        ...
        if(argc) {
            ...
            bool dart_enabled = STR_TO_BOOL(dart[0]);
            if(new_enabled != enabled) {
                ...
                if(new_enabled) {
                    ...
                    if (!dart_enabled) {
                        ...
                        overwrite(alpha_blend_camo_sig.data() + 4, static_cast<float>(0.9)); // Better camo with DART                        
                    }
